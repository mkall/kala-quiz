...
/**
 * Thanks to james-d for this code on github.
 * 
 * https://github.com/james-d/Virtual-keyboard
 */
public class VirtualKeyboard {
	...
	public VirtualKeyboard(ReadOnlyObjectProperty<Node> target) {
		...
		final String[][] shifted = new String[][] { { "~", "!", "@", "#", "$", "%", "^", "&", "*", "(", ")", "_", "+" },
				{ "Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P", "Å", "{", "}", "|" },
				{ "A", "S", "D", "F", "G", "H", "J", "K", "L", "Ö", "Ä", ":", "\"" },
				{ "Z", "X", "C", "V", "B", "N", "M", "<", ">", "?" } };
		...
		final KeyCode[][] codes = new KeyCode[][] {
				...
				{ KeyCode.Q, KeyCode.W, KeyCode.E, KeyCode.R, KeyCode.T, KeyCode.Y, KeyCode.U, KeyCode.I, KeyCode.O,
						KeyCode.P, KeyCode.UNDEFINED, KeyCode.OPEN_BRACKET, KeyCode.CLOSE_BRACKET, KeyCode.BACK_SLASH },
				{ KeyCode.A, KeyCode.S, KeyCode.D, KeyCode.F, KeyCode.G, KeyCode.H, KeyCode.J, KeyCode.K, KeyCode.L,
						KeyCode.UNDEFINED, KeyCode.UNDEFINED, KeyCode.SEMICOLON, KeyCode.QUOTE },
		...
		// final Button escape = createNonshiftableButton("Esc", KeyCode.ESCAPE,
		// modifiers, target);
		...
		// final Button enter = createNonshiftableButton("Enter", KeyCode.ENTER,
		// modifiers, target);
		// final Button tab = createNonshiftableButton("Tab", KeyCode.TAB, modifiers,
		// target);

		// "Extras" to go at the left or right end of each row of buttons.
		final Node[][] extraLeftButtons = new Node[][] { { /* escape */ }, { /* tab */ }, { /*
																							 * modifiers.capsLockKey()
																							 */ },
				{ modifiers.shiftKey() } };
		final Node[][] extraRightButtons = new Node[][] { { backspace }, { delete }, { /* enter */ },
				{ /* modifiers.secondShiftKey() */ } };

		Insets[] paddings = new Insets[unshifted.length];
		paddings[0] = new Insets(0, 0, 0, 10);
		paddings[1] = new Insets(0, 0, 0, 38);
		paddings[2] = new Insets(0, 0, 0, 46);
		paddings[3] = new Insets(0, 0, 0, 0);

		// build layout
		for (int row = 0; row < unshifted.length; row++) {
			HBox hbox = new HBox(5);
			hbox.setAlignment(Pos.CENTER_LEFT);
			hbox.setPadding(paddings[row]);
			...
			for (int k = 0; k < unshifted[row].length; k++) {
				hbox.getChildren().add(
						createShiftableButton(unshifted[row][k], shifted[row][k], codes[row][k], modifiers, target));
			}
